...
operm <- scan1perm(pr, biomass$pheno, kinship, addcovar=covar, cores=cores, n_perm=1000)
# operm <- readRDS(paste0(pop, ".operm.Rdata"))
summary(operm, alpha=c(0.05))
saveRDS(operm, paste0(pop, ".operm.Rdata"))
...
for(i in 1:num_phenotypes) {
    plot(out_pg, map, lodcolumn=i, col=color[1], type="l", pch=20, altcol="orange")
    legend("topleft", lwd=2, col=color, c("LMM"), bg="gray90", lty=c(1,1,2))
