...
covar <- maturity$maturity
names(covar) <- maturity$ID
...
out_loco <- readRDS(paste0(pop, ".loco.Rdata"))
if (paste0(pop, ".loco_operm.Rdata") %in% list.files()){
    operm <- readRDS(paste0(pop, ".loco_operm.Rdata"))
} else {
    operm <- scan1perm(pr, biomass$pheno, kinship_loco, addcovar=covar, cores=cores, n_perm=1000)
    summary(operm, alpha=c(0.05))
    saveRDS(operm, paste0(pop, ".loco_operm.Rdata"))
}
# operm <- readRDS(paste0(pop, ".operm.Rdata"))
out <- readRDS(paste0(pop, ".out.Rdata"))
out_pg <- readRDS(paste0(pop, ".lmm.Rdata"))

lod_peaks <- find_peaks(out_loco, map, threshold=3, peakdrop=1.8, drop=1.5)
print(lod_peaks)
...
# Plot operm
png(paste0(pop, ".operm.png"))
color <- c("slateblue")
par(mar=c(4.1, 4.1, 1.6, 1.1))
ymx <- maxlod(operm)
for(i in 1:num_phenotypes) {
    plot(out_loco, map, lodcolumn=i, col=color[1], type="l", pch=20, altcol="orange")
    legend("topleft", lwd=2, col=color, c("LOCO"), bg="gray90", lty=c(1,1,2))
