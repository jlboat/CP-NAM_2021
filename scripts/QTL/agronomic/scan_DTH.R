...
saveRDS(biomass$gmap, paste0(pop, ".map.DTH.Rdata"))
pr <- calc_genoprob(biomass, biomass$gmap, cores=cores)
saveRDS(pr, paste0(pop, ".genoprob.pr.DTH.Rdata"))
apr <- genoprob_to_alleleprob(pr, cores=cores)
saveRDS(apr, paste0(pop, ".alleleProb.apr.DTH.Rdata"))

kinship <- calc_kinship(pr, cores=cores)
saveRDS(kinship, paste0(pop, ".kinship.pr.DTH.Rdata"))

out <- scan1(pr, biomass$pheno, cores=cores)
saveRDS(out, paste0(pop, ".out.DTH.Rdata"))

# Process
peaks <- find_peaks(out, biomass$gmap, threshold=3, drop=1.5)
saveRDS(peaks, paste0(pop, ".peaks.DTH.Rdata"))
# bayes_int(out, map, lodcolumn=1, chr=3, prob=0.95)

out_pg <- scan1(pr, biomass$pheno, kinship, cores=cores)
saveRDS(out_pg, paste0(pop, ".lmm.DTH.Rdata"))

kinship_loco <- calc_kinship(pr, "loco", cores=cores)
saveRDS(kinship_loco, paste0(pop, ".loco_kinship.DTH.Rdata"))

out_pg_loco <- scan1(pr, biomass$pheno, kinship_loco, cores=cores)
saveRDS(out_pg_loco, paste0(pop, ".loco.DTH.Rdata"))

# Estimate heritability
herit <- est_herit(biomass$pheno, kinship)
saveRDS(herit, paste0(pop, ".herit.DTH.Rdata"))

# Plot H-K and LMM
for(i in 1:num_phenotypes) {
    png(paste0(pop, ".", i, ".DTH.H-K_LMM_LOCO.png"))
